...
export async function createPredictionMarketInStore(payload, authToken = null) {
  ...
  let { data: vibeRow } = await sb.from('vibes').select('id').eq('id', vibeId).maybeSingle();
  if (!vibeRow?.id) {
    ({ data: vibeRow } = await sb.from('vibes').select('id').eq('slug', vibeId).maybeSingle());
  }
  if (!vibeRow?.id) {
    return { created: false, reason: 'vibe_not_found' };
  }
  const resolvedVibeId = vibeRow.id;
  const { data: existingVibeMarket } = await sb
    .from('prediction_markets')
    .select('id')
    .eq('vibe_id', resolvedVibeId)
    .eq('state', 'open')
    .maybeSingle();
  ...
  const { markets } = await listPredictionMarkets({ state: 'all', authToken, limit: 200, vibeId: resolvedVibeId });
...
async function autoPayoutAllPositions(sb, marketRow) {
  const marketId = marketRow.id;
  if (!['resolved', 'cancelled'].includes(marketRow.state)) return { payoutCount: 0 };

  const isCancelled = marketRow.state === 'cancelled' || marketRow.resolved_outcome === 'cancelled';
  const winningSide = isCancelled ? null : (marketRow.resolved_outcome === 'yes' ? 'yes' : 'no');
  const totalPool = roundMoney(safeNumber(marketRow.yes_pool, 0) + safeNumber(marketRow.no_pool, 0));

  const [positionResult, claimResult] = await Promise.all([
    sb.from('prediction_market_positions').select('user_id, side, stake, shares, entry_probability').eq('market_id', marketId),
    sb.from('prediction_market_claims').select('user_id').eq('market_id', marketId),
  ]);

  const alreadyClaimed = new Set((claimResult?.data || []).map((r) => r.user_id));
  const allPositions = Array.isArray(positionResult?.data) ? positionResult.data : [];

  // Group by user
  const userMap = new Map();
  for (const pos of allPositions) {
    if (alreadyClaimed.has(pos.user_id)) continue;
    if (!userMap.has(pos.user_id)) {
      userMap.set(pos.user_id, { yesShares: 0, noShares: 0, totalStake: 0 });
    }
    const u = userMap.get(pos.user_id);
    if (pos.side === 'yes') u.yesShares = roundMoney(u.yesShares + safeNumber(pos.shares, 0));
    if (pos.side === 'no') u.noShares = roundMoney(u.noShares + safeNumber(pos.shares, 0));
    u.totalStake = roundMoney(u.totalStake + safeNumber(pos.stake, 0));
  }

  if (userMap.size === 0) return { payoutCount: 0 };
  ...
  if (winningSide) {
    const { data: winRows } = await sb
      ...
      .eq('side', winningSide);
    totalWinningShares = roundMoney((winRows || []).reduce((s, r) => s + safeNumber(r.shares, 0), 0));
  }

  const nowIso = new Date().toISOString();
  let payoutCount = 0;

  for (const [userId, up] of userMap.entries()) {
    let payout = 0;
    if (isCancelled) {
      payout = up.totalStake;
    } else if (winningSide === 'yes' && up.yesShares > 0 && totalWinningShares > 0) {
      payout = roundMoney((up.yesShares / totalWinningShares) * totalPool);
    } else if (winningSide === 'no' && up.noShares > 0 && totalWinningShares > 0) {
      payout = roundMoney((up.noShares / totalWinningShares) * totalPool);
    }

    const claimId = `claim-${marketId}-${userId}`;
    const { error: claimErr } = await sb.from('prediction_market_claims').upsert(
      { id: claimId, market_id: marketId, user_id: userId, amount: payout, created_at: nowIso },
      { onConflict: 'id', ignoreDuplicates: true }
    );
    if (claimErr) continue;

    if (payout > 0) {
      await adjustProfileAuraBalance(sb, userId, payout);
      // Award prediction_points equal to payout amount
      const { data: prof } = await sb.from('profiles').select('prediction_points').eq('id', userId).single();
      const pts = safeNumber(prof?.prediction_points, 0);
      await sb.from('profiles').update({ prediction_points: Math.round(pts + payout) }).eq('id', userId);
    }
    payoutCount++;
  }
  return { payoutCount };
...
async function determineMarketOutcome(sb, market) {
  if (!market.vibe_id) return null;

  const { data: vibe } = await sb.from('vibes').select('id, slug, end_time').eq('id', market.vibe_id).maybeSingle();
  if (!vibe) return null;

  const vaultId = `vault-${vibe.slug}`;
  const { data: vaultItem } = await sb.from('vault_items').select('id, price').eq('id', vaultId).maybeSingle();
  const isSettled = Boolean(vaultItem?.id);
  const settledPrice = isSettled ? safeNumber(vaultItem.price, 0) : null;

  const endTimeMs = new Date(vibe.end_time || '').getTime();
  const auctionEnded = Number.isFinite(endTimeMs) && endTimeMs <= Date.now();

  switch (market.market_type) {
    case 'binary':
      if (isSettled) return 'yes';
      if (auctionEnded) return 'no';
      return null;

    case 'price_target': {
      if (isSettled && settledPrice !== null) {
        const match = (market.title || '').match(/above\s+([\d,]+)/i);
        if (match) {
          const threshold = safeNumber(match[1].replace(/,/g, ''), Number.NaN);
          if (Number.isFinite(threshold)) return settledPrice > threshold ? 'yes' : 'no';
        }
        return null;
      }
      if (auctionEnded && !isSettled) return 'no';
      return null;
    }
    ...
    case 'engagement': {
      ...
      const { count } = await sb
        ...
        .eq('vibe_id', vibe.slug);
      const match = (market.title || '').match(/(\d[\d,]*)\s*bids/i);
      if (match) {
        const threshold = safeNumber(match[1].replace(/,/g, ''), Number.NaN);
        if (Number.isFinite(threshold)) return (count || 0) >= threshold ? 'yes' : 'no';
      }
      return null;
    }
...
export async function autoResolvePredictionMarkets() {
  const sb = getSupabaseAdmin();
  if (!sb) return { resolved: 0, skipped: 0, failed: 0 };

  const nowIso = new Date().toISOString();
  const { data: expiredMarkets, error } = await sb
    .from('prediction_markets')
    .select('*')
    .eq('state', 'open')
    .lt('closes_at', nowIso)
  ...
  if (error || !Array.isArray(expiredMarkets) || expiredMarkets.length === 0) {
  ...
  let skipped = 0;
  let failed = 0;
  const GRACE_PERIOD_MS = 24 * 60 * 60 * 1000; // 24h before auto-cancelling

  for (const market of expiredMarkets) {
    try {
      const outcome = await determineMarketOutcome(sb, market);

      if (outcome === null) {
        const closeMs = new Date(market.closes_at).getTime();
        if (Date.now() - closeMs > GRACE_PERIOD_MS) {
          // Auto-cancel after 24h grace period if still unresolvable
          const resolvedNow = new Date().toISOString();
          const { error: cancelErr } = await sb
            .from('prediction_markets')
            .update({ state: 'cancelled', resolved_outcome: 'cancelled', resolved_by: 'auto', resolved_at: resolvedNow, updated_at: resolvedNow })
            .eq('id', market.id)
            .eq('state', 'open');
          if (!cancelErr) {
            const updatedMarket = { ...market, state: 'cancelled', resolved_outcome: 'cancelled' };
            await autoPayoutAllPositions(sb, updatedMarket);
            resolved++;
          } else {
            failed++;
          }
      ...
      const resolvedNow = new Date().toISOString();
      const newState = outcome === 'cancelled' ? 'cancelled' : 'resolved';
      const { error: resolveErr } = await sb
        .from('prediction_markets')
        .update({ state: newState, resolved_outcome: outcome, resolved_by: 'auto', resolved_at: resolvedNow, updated_at: resolvedNow })
        .eq('id', market.id)
        .eq('state', 'open');

      if (resolveErr) { failed++; continue; }

      const updatedMarket = { ...market, state: newState, resolved_outcome: outcome };
      await autoPayoutAllPositions(sb, updatedMarket);
      resolved++;
    } catch {
      failed++;
...
export async function resolvePredictionMarketInStore(payload, authToken = null) {
  ...
  const updatedMarketRow = { ...marketRow, state: outcome === 'cancelled' ? 'cancelled' : 'resolved', resolved_outcome: outcome };
  await autoPayoutAllPositions(sb, updatedMarketRow);
...
export async function claimPredictionPayoutInStore(payload, authToken = null) {
  ...
  const { data: existingClaim } = await sb
    ...
    .maybeSingle();
  if (existingClaim) {
    return { claimed: true, amount: roundMoney(existingClaim.amount), autoClaimedAt: existingClaim.created_at };
  }
