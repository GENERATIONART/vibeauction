...
const computeYesProbability = (row) => {
  ...
  if (total > 0) return clamp(yesPool / total, 0.01, 0.99);
  const initial = parseProbability(row?.initial_probability, 0.5);
  return clamp(initial, 0.01, 0.99);
};
...
const mapMarketRow = ({ row, creatorHandle, participants, myPosition, myClaim }) => {
  ...
  return {
    ...
    probabilityYes,
    probabilityNo: roundMoney(1 - probabilityYes),
    initialProbability: parseProbability(row.initial_probability, 0.5),
...
export async function listPredictionMarkets({ state = 'all', authToken = null, limit = 100 } = {}) {
...
export async function createPredictionMarketInStore(payload, authToken = null) {
  ...
  const initialProbability = parseProbability(payload?.initialProbability, null);
  const seedLiquidity = roundMoney(Math.abs(safeNumber(payload?.seedLiquidity, 0)));
  const now = Date.now();

  if (!title || !closesAt || !Number.isFinite(initialProbability) || seedLiquidity < 10) {
  ...
  const yesPool = roundMoney(seedLiquidity * initialProbability);
  const noPool = roundMoney(seedLiquidity - yesPool);
  const marketId = normalize(`${title}-${Date.now()}`) || `market-${Date.now()}`;
  const debit = await adjustProfileAuraBalance(sb, userId, -seedLiquidity);
  if (!debit.ok) {
    return { created: false, reason: debit.reason || 'insufficient_balance' };
  }
  ...
  if (insertError) {
    await adjustProfileAuraBalance(sb, userId, seedLiquidity);
    return { created: false, reason: 'market_create_failed' };
  }

  const { markets } = await listPredictionMarkets({ state: 'all', authToken, limit: 200 });
...
export async function placePredictionTradeInStore(payload, authToken = null) {
  ...
  const probabilityYes = computeYesProbability(marketRow);
  const { price, shares } = calcSharesForStake({ side, stake, probabilityYes });
  ...
  const { error: insertPositionError } = await sb.from('prediction_market_positions').insert({
    ...
    stake,
    shares,
    entry_probability: probabilityYes,
    created_at: nowIso,
  });
