        ...
        if (bidderId && minimumBid === null && !bidFailureReason) {
          ...
          if (!bidFailureReason) {
            ...
            if (prevHighest?.user_id && prevHighest.user_id !== bidderId) {
              sendOutbidEmail({ toUserId: prevHighest.user_id, ...emailBase, newAmount: amount }).catch((e) => console.error('[email] outbid send failed:', e?.message ?? e));
            }

            // New bid on your vibe — notify the lister (if different from bidder)
            if (listerId && listerId !== bidderId) {
              sendNewBidOnYourVibeEmail({ toUserId: listerId, ...emailBase, bidAmount: amount, bidderHandle }).catch((e) => console.error('[email] new-bid send failed:', e?.message ?? e));
            }
          }
        }
      }
...
export async function mintVibeInStore(payload, authToken = null) {
  ...
  if (authToken) {
    ...
    if (sb) {
      ...
      }).catch((e) => console.error('[email] auth lookup failed:', e?.message ?? e));
    }
  }
