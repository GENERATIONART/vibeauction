...
import { buildGraduationSnapshot, GRADUATION_MAX_PER_WEEK, GRADUATION_WINDOW_DAYS } from '../vibe-graduation.js';
...
const toSlugMapKey = (value) => String(value || '').trim();

async function buildGraduationBoardRows(sb, { limit = 12 } = {}) {
  const windowStartMs = Date.now() - GRADUATION_WINDOW_DAYS * 24 * 60 * 60 * 1000;
  const { data: recentVibes, error: vibeError } = await sb
    .from('vibes')
    .select('id, slug, name, author, image_url, category, starting_price, created_at, end_time')
    .order('created_at', { ascending: false })
    .limit(300);

  if (vibeError) {
    throw new Error(vibeError.message || 'Failed to load graduation board');
  }

  const vibes = Array.isArray(recentVibes)
    ? recentVibes.filter((row) => row?.slug && toTimestampMs(row?.created_at) >= windowStartMs)
    : [];
  if (vibes.length === 0) return [];

  const slugs = vibes.map((row) => toSlugMapKey(row.slug)).filter(Boolean);
  const [{ data: bidRows }, { data: reactionRows }, { data: commentRows }, { data: remixRows }] = await Promise.all([
    sb
      .from('vibe_bids')
      ...
      .order('created_at', { ascending: false })
      .limit(8000),
    sb.from('vibe_reactions').select('vibe_id, user_id').in('vibe_id', slugs).limit(8000),
    sb.from('vibe_comments').select('vibe_id, user_id').in('vibe_id', slugs).limit(8000),
    sb.from('vibes').select('remix_source_slug').in('remix_source_slug', slugs).limit(8000),
  ]);
  ...
  const recentBidCountBySlug = new Map();

  for (const row of bidRows || []) {
    const slug = toSlugMapKey(row?.vibe_id);
    if (!slug) continue;
    ...
    if (Number.isFinite(createdAtMs) && createdAtMs >= windowStartMs) {
      recentBidCountBySlug.set(slug, (recentBidCountBySlug.get(slug) || 0) + 1);
    }
  }
  ...
  for (const row of remixRows || []) {
    const slug = toSlugMapKey(row?.remix_source_slug);
    if (!slug) continue;
    remixCountBySlug.set(slug, (remixCountBySlug.get(slug) || 0) + 1);
  }

  // Build unsorted entries with raw metrics (compute inputs only once)
  const entries = vibes.map((row) => {
    const slug = toSlugMapKey(row.slug);
    const inputs = {
      currentAura: Math.max(safeNumber(row.starting_price, 0), topBidBySlug.get(slug) || 0),
      ...
      recentBidCount: recentBidCountBySlug.get(slug) || 0,
    };
    const snapshot = buildGraduationSnapshot(inputs);
    return {
      slug,
  ...
  // Sort by score, then attach weeklyRank in a single final pass
  return entries
    .sort((a, b) => b._score - a._score || String(b.createdAt || '').localeCompare(String(a.createdAt || '')))
    .slice(0, Math.max(1, limit))
    .map((entry, index) => {
      const { _inputs, _score, ...rest } = entry;
      return { ...rest, graduation: buildGraduationSnapshot({ ..._inputs, weeklyRank: index + 1 }) };
    });
}
...
async function resolvePredictionsForAuction({ vibeId, finalPrice, actualWinnerTime }) {
  ...
  if (!normalizedId || settlementAmount <= 0 || !actualWinnerIso) {
    return { resolved: 0 };
  }
  ...
  // Batch-update all predictions at once using upsert
  await sb.from('auction_predictions').upsert(
    updates.map((u) => ({
      id: u.id,
      resolved: true,
      resolved_at: nowIso,
  ...
  if (userIds.length > 0) {
    ...
    await sb.from('profiles').upsert(
      userIds.map((userId) => ({
        id: userId,
        prediction_points: safeNumber(currentPointsByUser.get(userId), 0) + safeNumber(pointsByUserId.get(userId), 0),
      })),
      { onConflict: 'id' },
    );
  }
...
function getSupabaseAdmin() {
  ...
  if (!url || !key) {
    throw new Error('[state-db] SUPABASE_SERVICE_ROLE_KEY is not set — server-side DB operations require the service role key');
  }
...
async function fetchVibesFromSupabase() {
  try {
    ...
    const { data, error } = await sb
      .from('vibes')
      .select('id, slug, name, category, emoji, manifesto, duration, starting_price, buy_now_price, image_url, is_anonymous, author, listed_by, remix_source_slug, remix_source_name, remix_source_author, created_at, end_time')
      .order('created_at', { ascending: false })
      .limit(500);
...
export async function getRecentBidsForVibe(vibeId, limit = 10, vibeIdAlt = null, vibeName = null) {
  if (!vibeId) return { bids: [], topBid: null };
  let sb;
  try {
    sb = getSupabaseAdmin();
  } catch {
    return { bids: [], topBid: null };
  }
  const rawIds = [vibeId, vibeIdAlt]
    .filter((value) => value !== null && value !== undefined)
    .map((value) => String(value).trim())
    .filter(Boolean);
  // Include both raw and normalized forms to handle legacy writes.
  const candidateIds = [...new Set([
    ...rawIds,
    ...rawIds.map((value) => normalize(value)).filter(Boolean),
  ])];
  if (candidateIds.length === 0) return { bids: [], topBid: null };
  let recentRows = [];
  let topRows = [];

  const loadByIds = async () => Promise.all([
    sb
      .from('vibe_bids')
  ...
  const [{ data: byIdRecent, error: byIdRecentError }, { data: byIdTop, error: byIdTopError }] = await loadByIds();
  if (!byIdRecentError && !byIdTopError) {
    recentRows = Array.isArray(byIdRecent) ? byIdRecent : [];
    topRows = Array.isArray(byIdTop) ? byIdTop : [];
  }

  const safeVibeName = safeText(vibeName, 180);
  if (safeVibeName && recentRows.length === 0) {
    const [
      { data: byNameRecent, error: byNameRecentError },
      { data: byNameTop, error: byNameTopError },
    ] = await Promise.all([
      sb
        .from('vibe_bids')
        .select('amount, user_id, created_at')
        .ilike('vibe_name', safeVibeName)
        .order('created_at', { ascending: false })
        .limit(limit),
      sb
        .from('vibe_bids')
    ...
    if (!byNameRecentError && !byNameTopError) {
      recentRows = Array.isArray(byNameRecent) ? byNameRecent : [];
      topRows = Array.isArray(byNameTop) ? byNameTop : [];
    }
  }

  // Legacy compatibility: some older rows only match loosely on normalized name.
  if (safeVibeName && recentRows.length === 0) {
    const normalizedName = normalize(safeVibeName)
      .replace(/-/g, ' ')
      .trim();
    if (normalizedName) {
      const [
        { data: fuzzyRecent, error: fuzzyRecentError },
        { data: fuzzyTop, error: fuzzyTopError },
      ] = await Promise.all([
        sb
          .from('vibe_bids')
          .select('amount, user_id, created_at')
          .ilike('vibe_name', `%${normalizedName}%`)
          .order('created_at', { ascending: false })
          .limit(limit),
        sb
          .from('vibe_bids')
      ...
      if (!fuzzyRecentError && !fuzzyTopError) {
        recentRows = Array.isArray(fuzzyRecent) ? fuzzyRecent : [];
        topRows = Array.isArray(fuzzyTop) ? fuzzyTop : [];
      }
    }
  }
  ...
  if (userIds.length > 0) {
    const { data: profiles, error: profilesError } = await sb
      ...
      .select('id, username')
      .in('id', userIds);
    if (profilesError) console.error('[bids] profiles lookup failed:', profilesError.message);
    for (const p of profiles || []) usernameMap[p.id] = p.username;

    // For users with null/missing username, fall back to auth admin email prefix.
    const missingIds = userIds.filter((id) => !usernameMap[id]);
      ...
      await Promise.allSettled(
        missingIds.map(async (id) => {
          try {
            ...
            const raw = meta || email?.split('@')?.[0] || null;
            if (raw) {
              const cleaned = raw.toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 30);
              if (cleaned) usernameMap[id] = cleaned;
            }
          } catch {
            // leave usernameMap[id] undefined — will show 'Anonymous'
          }
        }),
      );
    }
...
export async function getPredictionForVibe(vibeId, authToken = null) {
  const normalizedId = normalize(vibeId);
  if (!normalizedId) return { prediction: null, stats: { totalPredictions: 0 } };
  let sb;
  try {
    sb = getSupabaseAdmin();
  } catch {
...
export async function getGraduationForVibe(vibeId, vibeIdAlt = null, vibeName = null) {
  const normalizedId = normalize(vibeId || vibeIdAlt || vibeName);
  if (!normalizedId) return { graduation: buildGraduationSnapshot({}) };
  ...
  try {
    ...
    return { graduation: buildGraduationSnapshot({}) };
  }
  ...
  const [
    ...
    sb
      .from('vibes')
      .select('slug, starting_price, created_at')
      .eq('slug', normalizedId)
      .maybeSingle(),
    sb
      .from('vibe_bids')
      ...
      .order('created_at', { ascending: false })
      .limit(1000),
    sb.from('vibe_reactions').select('user_id').in('vibe_id', candidateIds).limit(1000),
    sb.from('vibe_comments').select('user_id').in('vibe_id', candidateIds).limit(1000),
    sb.from('vibes').select('slug', { count: 'exact', head: true }).eq('remix_source_slug', normalizedId),
    buildGraduationBoardRows(sb, { limit: 200 }),
  ]);

  const currentAura = Math.max(
    safeNumber(vibeRow?.starting_price, 0),
    ...((bidRows || []).map((row) => safeNumber(row.amount, 0))),
  );
  const bidUserIds = new Set((bidRows || []).map((row) => row.user_id).filter(Boolean).map(String));
  const reactionUserIds = new Set((reactionRows || []).map((row) => row.user_id).filter(Boolean).map(String));
  const commentUserIds = new Set((commentRows || []).map((row) => row.user_id).filter(Boolean).map(String));
  const uniqueParticipants = new Set([...bidUserIds, ...reactionUserIds, ...commentUserIds]).size;
  const recentBidCount = (bidRows || []).filter((row) => new Date(row.created_at || '').getTime() >= Date.now() - 24 * 60 * 60 * 1000).length;

  const boardEntry = (boardRows || []).find((entry) => entry.slug === normalizedId);
  const weeklyRank = boardEntry?.graduation?.weeklyRank || null;

  return {
    graduation: buildGraduationSnapshot({
      currentAura,
      uniqueParticipants,
...
export async function getGraduationBoard(limit = 8) {
  let sb;
  try {
    sb = getSupabaseAdmin();
  } catch {
  ...
  const fullBoard = await buildGraduationBoardRows(sb, { limit: 200 });
  const board = fullBoard.slice(0, Math.max(limit, 1));
  return {
    board,
    summary: {
      breakoutCount: fullBoard.filter((entry) => entry.graduation?.state === 'breakout').length,
      graduatedCount: fullBoard.filter((entry) => entry.graduation?.state === 'graduated').length,
      qualifyingCount: fullBoard.filter((entry) => (entry.graduation?.weeklyRank || Infinity) <= GRADUATION_MAX_PER_WEEK).length,
    },
  };
}
...
async function fetchHighestBidsFromSupabase() {
  ...
  try {
    ...
    for (const row of data) {
      const key = normalize(row?.vibe_id || row?.vibe_name);
      if (!key) continue;
...
async function insertVaultItemToSupabase(userId, item) {
  if (!userId) return { inserted: false, reason: 'missing_supabase_or_user' };
  let sb;
  try {
    sb = getSupabaseAdmin();
  } catch {
    return { inserted: false, reason: 'missing_supabase_or_user' };
  }
...
async function deductAuraFromProfile(userId, amount) {
  ...
  // Use .eq filter on both id AND current balance to make this atomic-ish:
  // if another process already deducted, aura_balance will have changed and
  // this update will match 0 rows.
  const { data: updated, error: updateError } = await sb
    .from('profiles')
    .update({ aura_balance: current - deduction })
    .eq('id', userId)
    .eq('aura_balance', current)
    .select('aura_balance');

  if (updateError) return false;
  // If no rows updated, someone else changed the balance concurrently
...
export async function placeBidInStore(bid, authToken) {
  ...
  if (normalizedId && amount > 0) {
    ...
    if (sb) {
      ...
      const { data: vibeData } = await sb
        .from('vibes')
        .select('slug, listed_by, author, starting_price')
        .eq('slug', normalizedId)
        .single();
      ...
      if (authToken) {
        ...
        if (bidderId && minimumBid === null && !bidFailureReason) {
          // Get bidder's username for display, falling back to auth admin if null.
          const { data: bidderProfile } = await sb.from('profiles').select('username').eq('id', bidderId).single();
          let resolvedUsername = bidderProfile?.username ?? null;
          if (!resolvedUsername) {
            try {
              const { data: adminUser } = await sb.auth.admin.getUserById(bidderId);
              const meta = adminUser?.user?.user_metadata?.username;
              ...
              if (raw) {
                ...
                if (cleaned) {
                  ...
                  // Heal the null username in profiles for future lookups.
                  await sb.from('profiles').update({ username: cleaned }).eq('id', bidderId).is('username', null);
                }
              }
            } catch {
              // ignore — bidderHandle stays null
            }
          }
          const bidderHandle = resolvedUsername ? `@${resolvedUsername}` : null;
          ...
          if (!bidFailureReason) {
            // Get vibe info for emails (slug, listed_by)
            const vibeSlug = vibeRow?.slug ?? normalizedId;
            const emailBase = { vibeName: bid?.name || 'Unknown Vibe', vibeSlug };

            // Await emails so serverless functions don't terminate before Resend finishes the request
            const emailPromises = [];
            if (prevHighest?.user_id && prevHighest.user_id !== bidderId) {
              emailPromises.push(sendOutbidEmail({ toUserId: prevHighest.user_id, ...emailBase, newAmount: amount }));
            }
            if (listerId && listerId !== bidderId) {
              emailPromises.push(sendNewBidOnYourVibeEmail({ toUserId: listerId, ...emailBase, bidAmount: amount, bidderHandle }));
            }
  ...
  return updateState((state) => {
    ...
    const nextEntry = {
      id: normalizedId,
      emoji: bid?.emoji || null,
      imageUrl: bid?.imageUrl ?? null,
...
export async function settleAuctionInStore(item, authToken) {
  ...
  if (userId) {
    ...
    if (sb) {
      const { data: vibeData } = await sb
        .from('vibes')
        .select('name, category, image_url, author, buy_now_price, end_time')
        .eq('slug', normalizedId)
        .single();
    ...
    // Await so serverless functions don't terminate before Resend finishes
    ...
    }).catch((e) => console.error('[email] auction-won send failed:', e?.message ?? e));
...
export async function mintVibeInStore(payload, authToken = null) {
  ...
  if (sb) {
    ...
    const userId = userData?.user?.id ?? null;
    if (!userId) {
      const authMessage = String(userError?.message || '').toLowerCase();
      const authStatus = Number(userError?.status || 0);
      ...
      return {
        state,
        mintedVibe: null,
        reason: userError ? (looksLikeInvalidToken ? 'auth_invalid_token' : 'auth_lookup_failed') : 'auth_required',
      };
    }
  ...
  if (authToken) {
    let sbEmail;
    try { sbEmail = getSupabaseAdmin(); } catch { sbEmail = null; }
    if (sbEmail) {
      const sb = sbEmail;
      try {
        const { data } = await sb.auth.getUser(authToken);
        ...
        if (userId) {
          await sendVibeListedEmail({
            toUserId: userId,
            vibeName: mintedVibe.name,
            vibeEmoji: mintedVibe.emoji,
            vibeSlug: mintedVibe.slug,
            startingPrice: mintedVibe.startingPrice,
          }).catch((e) => console.error('[email] vibe-listed send failed:', e?.message ?? e));
        }
      } catch (e) {
        console.error('[email] auth lookup failed:', e?.message ?? e);
      }
    }
  }
