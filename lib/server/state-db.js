...
export async function getRecentBidsForVibe(vibeId, limit = 10, vibeIdAlt = null, vibeName = null) {
  ...
  let [{ data: recentRows, error: recentError }, { data: topRows, error: topError }] = await Promise.all([
    sb
      .from('vibe_bids')
  ...
  if (safeVibeName && (!Array.isArray(recentRows) || recentRows.length === 0)) {
    const [
