...
export async function getRecentBidsForVibe(vibeId, limit = 10, vibeIdAlt = null) {
  const sb = getSupabaseAdmin();
  if (!sb || !vibeId) return { bids: [], topBid: null };
  // Normalize IDs so lookup matches how bids are stored (via normalize() in placeBidInStore)
