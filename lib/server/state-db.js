...
import { auctionItems } from '../auction-items.js';
...
async function fetchVibesFromSupabase() {
  ...
  return data
    .filter((row) => hasNonBlankImageUrl(row?.image_url))
    .map((row) => ({
    id: row.id,
    slug: row.slug,
    name: row.name,
...
async function fetchHighestBidsFromSupabase() {
  ...
  try {
    ...
    for (const row of data) {
      if (!row?.vibe_id || highestByVibe.has(row.vibe_id)) continue;
  ...
  return queueStateTask(async () => {
    ...
    if (Array.isArray(supabaseVibes)) {
      for (const vibe of supabaseVibes) {
        if (!hasNonBlankImageUrl(vibe?.imageUrl)) continue;
...
export async function placeBidInStore(bid, authToken) {
  ...
  if (normalizedId && amount > 0) {
    ...
    if (sb) {
      ...
      if (!vibeRow) {
        // Fall back to static auction items (not in Supabase but still valid for bidding)
        const staticItem = auctionItems.find((item) => normalize(item.slug) === normalizedId);
        if (staticItem) {
          vibeRow = { slug: normalizedId, listed_by: null, author: null, starting_price: staticItem.bid || 0 };
        } else {
