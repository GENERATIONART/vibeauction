...
async function buildGraduationBoardRows(sb, { limit = 12 } = {}) {
  const windowStartIso = new Date(Date.now() - GRADUATION_WINDOW_DAYS * 24 * 60 * 60 * 1000).toISOString();
  ...
  const vibes = Array.isArray(recentVibes) ? recentVibes.filter((row) => row?.slug) : [];
