      ...
      if (authToken) {
        ...
        if (bidderId && minimumBid === null) {
          ...
          const { data: vibeRow } = await sb
            .from('vibes')
            .select('slug, emoji, listed_by, author')
          ...
          const listerId = vibeRow?.listed_by ?? null;
  ...
  if (userId) {
    ...
    if (price > 0) {
      ...
      if (sb) {
        ...
        if (currentBalance < Math.abs(price)) {
    ...
    const deducted = await deductAuraFromProfile(userId, price);
    if (!deducted) {
    ...
    sendAuctionWonEmail({
      toUserId: userId,
      vibeName: item?.name || 'Unknown Vibe',
      vibeEmoji: item?.emoji || '✨',
