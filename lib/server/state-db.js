...
import { buildGraduationSnapshot, GRADUATION_WINDOW_DAYS } from '../vibe-graduation.js';
...
export async function getGraduationForVibe(vibeId, vibeIdAlt = null, vibeName = null) {
  ...
  const weeklySnapshots = (weeklyVibeRows || []).map((row) => {
    const slug = String(row?.slug || '').trim();
    const isTarget = slug === normalizedId;
    return {
      slug,
      snapshot: buildGraduationSnapshot({
        currentAura: isTarget ? currentAura : safeNumber(row?.starting_price, 0),
        uniqueParticipants: isTarget ? uniqueParticipants : 0,
        totalReactions: isTarget ? (reactionRows || []).length : 0,
        commentCount: isTarget ? (commentRows || []).length : 0,
        remixCount: isTarget ? (remixCount || 0) : 0,
        recentBidCount: isTarget ? recentBidCount : 0,
      }),
    };
  }).sort((a, b) => b.snapshot.score - a.snapshot.score);

  const weeklyRankIndex = weeklySnapshots.findIndex((entry) => entry.slug === normalizedId);
