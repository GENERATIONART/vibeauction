...
import { claimAgentInStore } from '../../../../lib/server/agent-db.js';
...
export async function POST(request) {
  try {
    ...
    const result = await claimAgentInStore(body?.claimToken, actor.userId);
    return NextResponse.json(result, { status: result.claimed ? 200 : 400 });
  } catch {
